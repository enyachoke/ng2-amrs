...
export class GroupDetailComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public loadGroup() {
    ...
    this.subscriptions.add(this.communityGroupService.getGroupByUuid(uuid).subscribe((res) => {
      ...
      this.cohortVisits = res.cohortVisits;
  ...
  public gridOnCellClick($event) {
    ...
    let programUuid = this.patientDashboardConfig.programs.find((a) => {
      return a.alias === programShortName;
    }
    ).programUuid;

    if (programUuid && personUuid) {
      // this.patientVisitPayload = {
      ...
      //   visit: {
      //     patient: personUuid,
      //     location: this.patientVisitPayload.location.uuid,
      //     startDatetime: this.patientVisitPayload.cohortVisit.startDate,
      //     visitType: this.patientVisitPayload.cohortVisit.visitType.uuid,
      //   },
      //   cohortVisit: this.patientVisitPayload.cohortVisit.uuid
      // };
      this.router.navigate([`/patient-dashboard/patient/${personUuid}/`,
        'hiv', programUuid, 'visit'], { queryParams: { groupVisitUuid, groupUuid: this.group } });
